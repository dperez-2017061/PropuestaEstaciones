...
export class MapaGeneralComponent implements OnInit {
  ...
  createMarker(location: any, place: any, map: any) {
    ...
    }else if(place.type == 'Bombero Voluntario'){
      img = '../../assets/img/volunteerFire.png'
    }else if(place.type == 'Policia Nacional'){
      img = '../../assets/img/nationalPolice.png'
    }else{
      img = '../../assets/img/municipalPolice.png'
    }
    const marker = new google.maps.Marker({
      map,
      position: location,
    ...
    if (this.userRest.getIdentity().role == 'ADMIN') {
      ...
      content =
        ...
        `<h3><b>Usuario que la creo:</b> ${place.user.name}</h3>` +
  ...
  getStations(map: any) {
    this.stationRest.getStations().subscribe({
      next: (res: any) => {
